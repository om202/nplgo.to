...
function useRotatingText(phrases: string[], interval: number = 3000) {
    ...
    }, [phrases.join(','), interval]); // Use join(',') for stable dependency or just interval if phrases are constant

    return { text: phrases[index], isVisible };
}
...
export default function Home() {
    ...
    const { text: rotatingTitle, isVisible } = useRotatingText(HERO_PHRASES);
        ...
        <Layout>
            ...
            <section className="relative text-center -mt-20 pt-24 sm:pt-32 pb-16 sm:pb-20 md:pb-24">
                ...
                <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 md:px-8">
                    ...
                    <h1
                        className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight leading-tight mt-5 max-w-2xl mx-auto flex flex-col items-center"
                        style={{
                        ...
                        <div className="flex items-center gap-x-2">
                            ...
                            <div className="inline-grid text-left">
                                {HERO_PHRASES.map((phrase) => (
                                    <span key={phrase} className="invisible pointer-events-none row-start-1 col-start-1 whitespace-nowrap">
                                        {phrase}
                                    </span>
                                ))}
                                <span className={`text-primary row-start-1 col-start-1 transition-all duration-500 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
                                    {rotatingTitle}
                                </span>
                            </div>
                        </div>
                        <span className="text-2xl sm:text-3xl md:text-4xl font-semibold text-muted-foreground mt-1">
                            for Nepal
                        </span>
                    </h1>
                ...
                <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 md:px-8">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 py-1 text-center">
                        <div className="space-y-1">
                            <p className="text-2xl sm:text-3xl font-bold tracking-tight text-primary">1000+</p>
